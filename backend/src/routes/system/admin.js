...
const HOTSPOT_SOURCE_CONFIG = {
  rides: {
    label: '骑行热点',
    description: '基于骑行轨迹中点生成热点网格',
    metricLabel: '距离(km)',
    countSql: `
      SELECT COUNT(*)::int AS count
      FROM ride_sessions
      WHERE track_geom IS NOT NULL AND status = 'completed'
    `,
    baseSql: `
      SELECT
        ST_LineInterpolatePoint(track_geom, 0.5) AS geom,
        total_distance::numeric AS metric_value,
        CONCAT('骑行#', id) AS label,
        start_time AS stat_time
      FROM ride_sessions
      WHERE track_geom IS NOT NULL AND status = 'completed'
        AND ($1::date IS NULL OR DATE(start_time) = $1::date)
        AND ($2::int IS NULL OR EXTRACT(HOUR FROM start_time) = $2::int)
    `
  },
  reports: {
    label: '事件热点',
